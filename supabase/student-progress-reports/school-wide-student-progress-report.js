...
console.info('School-wide progress reports function started')
...
Deno.serve(async req => {
  ...
  try {
    ...
    if (!academic_year) {
      throw new Error('academic_year is required (format: "2024-2025")')
    }
    ...
    console.log(
      `Processing school-wide progress reports for school: ${school_id}, year: ${academic_year}`,
    )
    ...
    // 3. Get all speech screenings with proper joins for this school within the academic year
    const screeningsUrl = `${supabaseUrl}/rest/v1/speech_screenings?students.school_id=eq.${school_id}&select=*,students!inner(*),school_grades(*)&order=created_at.asc`
    ...
    if (!screeningsResponse.ok) {
      throw new Error(`Failed to fetch screenings: ${screeningsResponse.status}`)
    }

    const allScreenings = await screeningsResponse.json()

    // 4. Filter screenings for this academic year, excluding absent results
    const schoolScreenings = allScreenings.filter(
      screening =>
        screening.students &&
        isWithinAcademicYear(screening.created_at, academic_year) &&
        screening.result !== 'absent' &&
        screening.error_patterns?.attendance?.absent !== true,
    )

    console.log(`Found ${schoolScreenings.length} screenings for this school and year`)

    // 5. Group screenings by student
    const studentScreeningsMap = new Map()

    for (const screening of schoolScreenings) {
      const studentKey = `${screening.students.first_name}-${screening.students.last_name}-${screening.students.school_id}`

      if (!studentScreeningsMap.has(studentKey)) {
        studentScreeningsMap.set(studentKey, [])
      }
      studentScreeningsMap.get(studentKey).push(screening)
    }

    // 6. Process each student to create progress reports
    const documentObjects = []

    for (const [studentKey, screenings] of studentScreeningsMap) {
      // Sort screenings by date
      screenings.sort((a, b) => new Date(a.created_at) - new Date(b.created_at))

      // Need at least 2 screenings for a progress report
      if (screenings.length < 2) {
        console.log(`Skipping student ${studentKey} - only ${screenings.length} screening(s)`)
        continue
      }

      const current = screenings[screenings.length - 1]
      const previous = screenings[0]

      if (!previous) {
        console.log(`Skipping student ${studentKey} - no valid previous screening`)
        continue
      }

      const [startYear] = academic_year.split('-').map(Number)
      const threshold = new Date(startYear, 6, 1) // July 1 of academic year start

      if (new Date(previous.created_at) < threshold) {
        console.log(`Skipping student ${studentKey} - previous screening outside 13-month window`)
        continue
      }

      console.log(
        `Processing student: ${current.students.first_name} ${current.students.last_name}`,
      )

      // 7. Process error patterns for current and previous screenings
      const currentErrors = processErrorPatterns(current.error_patterns || {})
      const previousErrors = processErrorPatterns(previous.error_patterns || {})

      // 8. Build primary table — one row per (sound, pattern), joined across both screenings
      const primaryTableErrors = buildPrimaryTableErrors(previousErrors, currentErrors)

      // 9. Extract student info
      const studentInfo = {
        first_name: current.students.first_name,
        last_name: current.students.last_name,
        grade: current.school_grades?.grade_level || '',
        school: school.name,
      }

      // 10. Extract progress_notes from current screening
      const progressNotes = current.progress_notes || ''

      // 11. Format screening dates
      const initialScreenDate = formatDate(previous.created_at)
      const progressScreenDate = formatDate(current.created_at)

      // 12. Create document object
    ...
    // 12. Check if we have any documents to process
    if (documentObjects.length === 0) {
      return new Response(
        JSON.stringify({
          success: false,
          message: 'No students qualified for progress reports in this batch',
    ...
    console.log(`Generated ${documentObjects.length} progress reports`)

    // 13. Create a password-protected report token instead of emailing the PDFs directly
    ...
    const token = await createReportToken({
      ...
      supabaseKey,
      password,
      reportType: 'school_wide_progress_reports',
      reportData: {
    ...
    await sendReportLinkEmail({
      ...
      subject: `${schoolName} - Progress Reports - ${academic_year}`,
      reportLabel: `${schoolName}'s progress reports for ${academic_year}`,
      viewUrl,
    })

    console.log(`Secure report link sent successfully`)

    // 15. Log the generation in reports table for each student
    const reportInserts = documentObjects.map(doc => ({
      student_id: null, // You'll need to map this if you want individual student tracking
      school_id: school_id,
      speech_screening_id: null, // Multiple screenings involved
      ...
      metadata: {
        ...
        student_count: documentObjects.length,
        academic_year: academic_year,
    ...
    await fetch(reportInsertUrl, {
      ...
      body: JSON.stringify(reportInserts[0]), // Insert one record representing the batch
    })

    return new Response(
      JSON.stringify({
        success: true,
        message: `School-wide progress reports generated and a secure link sent to ${override_emails.join(', ')}`,
        school_name: school.name,
        student_count: documentObjects.length,
        academic_year: academic_year,
      }),
    ...
    console.error('Error generating school-wide progress reports:', error)
    return new Response(
      JSON.stringify({
        error: error.message,
        success: false,
...
function buildPrimaryTableErrors(previousErrors, currentErrors) {
  ...
  for (const key of allKeys) {
    ...
    merged.push({
      ...
      initial_screen: formatStimulability(prev?.stimulability_option),
      progress_screen: formatStimulability(curr?.stimulability_option),
      summary,
    })
  }
...
function createDocumentObject({
  ...
  return {
    metadata: {
      file_name: `${studentInfo.first_name}_${studentInfo.last_name}-PR`,
