...
const Notifications = () => {
  ...
  const toggleSelectAll = () => {
    ...
    if (allSelected) {
      setSelectedNotifications(prev =>
        prev.filter(id => !currentTabNotifications.find(n => n.id === id))
      )
    } else {
      setSelectedNotifications(prev => [
        ...new Set([...prev, ...currentTabNotifications.map(n => n.id)]),
      ])
    }
  }
        ...
        <div className='flex items-center justify-between mb-6'>
          ...
          <div className='flex gap-2'>
            <Button variant='outline' size='sm' onClick={toggleSelectAll}>
              {getTabNotifications(activeTab).every(n => selectedNotifications.includes(n.id))
                ? 'Deselect All'
                : 'Select All'}
            </Button>
          ...
          {urgentNotifications.length === 0 ? (
            <div className='text-center py-12'>
              ...
              <p className='text-gray-500'>
                You have no urgent notifications that require immediate attention
              </p>
            </div>
          ) : (
