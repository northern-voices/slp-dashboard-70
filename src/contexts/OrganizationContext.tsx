...
export const OrganizationProvider: React.FC<OrganizationProviderProps> = ({ children }) => {
  ...
  const refreshData = useCallback(
    async (force: boolean = false) => {
      ...
      // Prevent excessive fetching - only fetch if forced or if data is stale (older than 5 minutes)
      const now = Date.now()
      const isStale = now - lastFetchTime > 5 * 60 * 1000 // 5 minutes

      if (!force && !isStale && isInitialized) {
        return
      }
      ...
      try {
        ...
        // Mark as initialized and update fetch time
    ...
    [user, clearCurrentSchool, lastFetchTime, isInitialized]
  )

  // Only fetch data once when user changes, not on every render
  useEffect(() => {
    if (user?.id && !isInitialized) {
      refreshData(true)
    }
  }, [user?.id, isInitialized]) // Remove refreshData from dependencies to prevent circular calls

  // Handle initial localStorage loading - only run once after schools are loaded
  useEffect(() => {
    if (user && !isLoading && availableSchools.length > 0 && isInitialized) {
  ...
  }, [user, isLoading, availableSchools, currentSchool, clearCurrentSchool, isInitialized])
