...
const DraftsList = ({
  ...
  const filteredDrafts = drafts.filter(draft => {
    ...
    const matchesUser =
      filterUser === 'all' || (filterUser === 'mine' && draft.user_id === currentUserId)

    return matchesSearch && matchesType && matchesUser
  })

  const canEdit = (draft: Draft) => {
    return draft.user_id === currentUserId || userRole === 'admin' || userRole === 'super_admin'
  }
  ...
  return (
    <div className='space-y-6'>
      ...
      <div className='flex gap-3'>
        <Button onClick={() => navigate('/screening/speech')} className='flex items-center gap-2'>
