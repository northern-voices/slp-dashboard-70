
import { Screening } from '@/types/database';

export const mockScreenings: Screening[] = [
  {
    id: '1',
    student_id: '1',
    type: 'speech',
    status: 'completed',
    date: '2024-01-20',
    screening_date: '2024-01-20',
    screening_type: 'initial',
    screener: 'Dr. Sarah Johnson',
    slp_id: 'slp1',
    notes: 'Initial speech screening - identified articulation concerns',
    created_at: '2024-01-20T10:00:00Z',
    updated_at: '2024-01-20T10:00:00Z',
  },
  {
    id: '2',
    student_id: '1',
    type: 'speech',
    status: 'completed',
    date: '2023-11-15',
    screening_date: '2023-11-15',
    screening_type: 'progress',
    screener: 'Dr. Sarah Johnson',
    slp_id: 'slp1',
    notes: 'Progress screening - significant improvement in /r/ sound production',
    created_at: '2023-11-15T10:00:00Z',
    updated_at: '2023-11-15T10:00:00Z',
  },
  {
    id: '3',
    student_id: '1',
    type: 'speech',
    status: 'completed',
    date: '2023-08-10',
    screening_date: '2023-08-10',
    screening_type: 'progress',
    screener: 'Dr. Sarah Johnson',
    slp_id: 'slp1',
    notes: 'Progress screening - continued speech therapy recommended',
    created_at: '2023-08-10T10:00:00Z',
    updated_at: '2023-08-10T10:00:00Z',
  },
  {
    id: '4',
    student_id: '1',
    type: 'speech',
    status: 'completed',
    date: '2023-03-22',
    screening_date: '2023-03-22',
    screening_type: 'progress',
    screener: 'Dr. Sarah Johnson',
    slp_id: 'slp1',
    notes: 'Teacher referral for speech evaluation',
    created_at: '2023-03-22T10:00:00Z',
    updated_at: '2023-03-22T10:00:00Z',
  },
];
