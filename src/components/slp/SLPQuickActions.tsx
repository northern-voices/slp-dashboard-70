...
const SLPQuickActions = () => {
  ...
  const quickActions = [
    ...
    {
      title: 'View Students',
      description: selectedSchool ? `Browse students at ${selectedSchool.name}` : 'Select a school to view students',
      icon: Users,
      ...
      <CardHeader className="pb-4">
        ...
        {!selectedSchool && (
          <p className="text-sm text-gray-600">
            Select a school above to enable screening actions
          </p>
        )}
      </CardHeader>
      <CardContent className="pt-0">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {quickActions.map((action, index) => (
            <Button
              key={index}
              className={`h-auto p-4 flex flex-col items-center space-y-2 ${action.color} text-white transition-all duration-200 ${
