...
import { Plus, Users, FileText, Settings, Mic, Volume2, BarChart3 } from 'lucide-react';
const QuickActions = () => {
  const actions = [{
  ...
  return <Card className="mb-10 bg-white border-gray-100 shadow-sm">
      ...
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {actions.map((action, index) => <Button key={index} variant="outline" className="h-auto p-6 flex flex-col items-center space-y-4 hover:shadow-md transition-all duration-200 hover:-translate-y-0.5 border-gray-200 bg-white hover:bg-gray-50" onClick={action.action}>
